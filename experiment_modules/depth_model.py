...
from losses import MSGradientLoss, MVDepthLoss, NormalsLoss, ScaleInvariantLoss,UncertaintyLoss
...
class DepthModel(pl.LightningModule):
    ...
    def __init__(self, opts):
        ...
        #Acmix block
        self.Acmix = nn.ModuleDict()
        cur_feats_dim=[24,48,64,160,256]
        for i in range(len(cur_feats_dim)):
            self.Acmix[f"cur_feats_{i}"]=ACmix(in_planes=cur_feats_dim[i], out_planes=cur_feats_dim[i])
    ...
    def forward(
        ...
        for i in range(len(cur_feats)):
            #print("cur_feats:",i,cur_feats[i].size())
            cur_feats[i] = self.Acmix[f"cur_feats_{i}"](cur_feats[i])
        #print("cur_feats:",len(cur_feats),cur_feats[0].size())
        ...
        #print("cur_feats:",len(cur_feats),cur_feats[0].size())
        #print("matching_cur_feats:",len(matching_cur_feats),matching_cur_feats[0].size())
    ...
    def compute_losses(self, cur_data, src_data, outputs):
        ...
        for i in range(4):
            ...
            if f"uncert_s{i}_b1hw" in outputs :
                # 获取通道数
                c = outputs[f"uncert_s{i}_b1hw"].size(1) // 2

                # 在通道维度上切分成 mean 和 logvar
                outputs_0,outputs_1 = torch.split(outputs[f"uncert_s{i}_b1hw"], c, dim=1)
                uncert_resized_0 = F.interpolate(
                    outputs_0,
                    size=depth_gt.shape[-2:],
                    mode="nearest",
                )
                uncert_resized_1 = F.interpolate(
                    outputs_1,
                    size=depth_gt.shape[-2:],
                    mode="nearest",
                )
            unc_loss+=self.uncert_loss(uncert_resized_0[mask_b],uncert_resized_1[mask_b],log_depth_pred_resized[mask_b],log_depth_gt[mask_b])
        #print("unc_loss",unc_loss)
        ...
        loss = 1.0*ms_loss + 1.0 * grad_loss + 1.0 * normals_loss + 0.3 * mv_loss +0.3 * unc_loss 
        #print("ms_loss:",ms_loss,"grad_loss:",grad_loss,"normals_loss:",normals_loss,"mv_loss:",mv_loss,"unc_loss ",unc_loss )
        losses = {
            "loss": loss,
            "si_loss": si_loss,
            "grad_loss": grad_loss,
