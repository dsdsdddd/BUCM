...
class UncertaintyLoss(nn.Module):
    ...
    def entropy_pytorch(self,probabilities_tensor):
        # Avoid log(0) by adding a small epsilon
        epsilon = 1e-4
        probabilities_tensor = torch.clamp(probabilities_tensor, epsilon, 1.0 - epsilon)
        # Calculate entropy using the base-2 logarithm
        entropy = -torch.sum(probabilities_tensor * torch.log2(probabilities_tensor))/40

        return entropy

    def forward(self,pred, gt, uct):
        #uct = torch.clamp(uct, max=1)
        #entropy=self.entropy_pytorch(uct)
        abs_diff = torch.abs(pred - gt)
        loss = (abs_diff / (torch.exp(uct))) + uct #* torch.log2(entropy)
        #loss = torch.mean(loss, dim=[1, 2, 3])
        loss = torch.mean(loss)/10#* torch.log2(entropy)
        return loss'''
...
class UncertaintyLoss(nn.Module):
    ...
    def entropy_pytorch(self, probabilities_tensor):
        epsilon = 1e-4
        probabilities_tensor = torch.clamp(probabilities_tensor, epsilon, 1.0 - epsilon)
        entropy = -torch.mean(probabilities_tensor * torch.log2(probabilities_tensor))
        return entropy

    def forward(self, pred_mean, pred_logvar, estimated_value, gt):
        # 计算均值的平方作为方差的估计
        pred_var = torch.exp(pred_logvar)
        
        # 计算平方差
        squared_diff_mean = torch.pow(pred_mean - estimated_value, 2)
        squared_diff_estimated = torch.pow(estimated_value - gt, 2)
        
        # 计算损失
        loss = squared_diff_mean / (2 * pred_var) + 0.5 * pred_logvar
        loss = torch.mean(loss)
        #print("loss_stage1:",loss)
        
        # 添加不确定性的熵项
        entropy = self.entropy_pytorch(torch.exp(-0.5 * pred_logvar))
        #print("loss_stage2:",entropy)
        loss += entropy
        
        # 加入估计值与真实值的平方差
        loss += torch.mean(squared_diff_estimated)
