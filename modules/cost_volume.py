...
from modules.function import EVCBlock
...
class CostVolumeManager(nn.Module):
    ...
    def get_mask(self, pix_coords_bk2hw):
        ...
        """
        创建一个掩码(mask),以忽略源图像边缘或外部的特征
        Args:
            pix_coords_bk2hw: 源特征的采样位置      
        Returns:
    ...
    def generate_depth_planes(self, batch_size: int, 
        ...
        log_depth_planes_bd11 = (torch.log(min_depth) + 
                                    torch.log(max_depth / min_depth) 
                                        * linear_ramp_bd11)
        #应用指数函数torch.exp，得到深度平面张量
        depth_planes_bd11 = torch.exp(log_depth_planes_bd11)
        #将 depth_planes_bd11 张量扩展到指定的大小
    ...
    def warp_features(
        ...
        将每个源视图特征扭曲到由 depth_plane_b1hw 定义的深度平面的当前视图。

        Args:
            src_feats: 源图像的匹配特征，其形状为 B x num_src_frames x C x H x W
            src_extrinsics: 源图像相机的外参，相对于当前相机，其形状为 B x num_src_frames x 4 x 4
            src_Ks: 源图像的逆内参，其形状为 B x num_src_frames x 4 x 4
            cur_invK: 当前图像的逆内参，其形状为 B x 4 x 4
            depth_plane_b1hw:用于每个空间位置的深度平面
            batch_size: the batch size.
            num_src_frames: number of source views.
            num_feat_channels: number of feature channels for feature maps.
            uv_scale: 在 grid_sample 之前对图像空间坐标进行归一化的值.
        ...
        #将 cam_points_B3N 张量重塑为 cam_points_B3hw
        cam_points_B3hw = cam_points_B3N.view(-1, 3, self.matching_height, 
                                                            self.matching_width)
        
        #提取像素坐标 pix_coords_B2hw 和深度 depths
...
class FeatureVolumeManager(CostVolumeManager):
    ...
    def build_cost_volume(self, 
        ...
        for depth_id in range(self.num_depth_bins):
            ...
            '''
            # run through the MLP!
            mlp_input_features_bchw=self.evc(mlp_input_features_bchw)
            # print("mlp_input_features_bchw:",mlp_input_features_bchw.size())
            mlp_input_features_bhwc = mlp_input_features_bchw.permute(0, 2, 3, 1)
            #print("mlp_input_features_bhwc:",mlp_input_features_bhwc.size())
